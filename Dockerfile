...
WORKDIR /go/src/github.com/node/xrplevm
COPY . .
...
RUN tar -xf /go/src/github.com/node/xrplevm/release/exrp_${VERSION}_linux_amd64.tar.gz -C /usr/bin
...
# Unit tests
RUN go test $(go list ./... | grep -v github.com/node/xrplevm/tests/e2e/poa)
# End to end tests
...
COPY --from=build /go/src/github.com/node/xrplevm/release /binaries
